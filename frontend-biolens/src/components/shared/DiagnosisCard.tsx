...
import { PLANT_EMOJIS, formatDiagnosisDate, getDiagnosisBadgeColor } from '@/utils/dashboard'
...
export const DiagnosisCard = ({ diagnosis }: DiagnosisCardProps) => {
  const emoji = PLANT_EMOJIS[diagnosis.plantName] ?? '🌿'
  const badgeColor = getDiagnosisBadgeColor(diagnosis.isHealthy)
  const badgeLabel = diagnosis.isHealthy ? 'Healthy' : (diagnosis.diseaseName ?? 'Diseased')
    ...
    >
      ...
      <VStack align="flex-start" gap="0" flex="1" minW="0">
        <Text fontWeight="bold" fontSize="sm" truncate>
          {diagnosis.plantName} — {fieldLabel}
        </Text>
        <Text fontSize="xs" color="gray.400">
          {formatDiagnosisDate(diagnosis.diagnosedAt)}
          {diagnosis.confidence !== null ? ` · ${Math.round(diagnosis.confidence)}% confidence` : ''}
        </Text>
      </VStack>
